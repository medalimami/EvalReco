...
class OracleNMF(VirtualOracle):
    def __init__(self, num_users, num_items, rank=10, min_rating = 1.0, max_rating=5.0):
        self.num_users = num_users
        self.num_items = num_items
        self.rank = rank

        minIndividuel = math.sqrt(min_rating / rank)
        maxIndividuel = math.sqrt(max_rating / rank)

        self.A = minIndividuel + np.random.rand(num_users, rank) * (maxIndividuel - minIndividuel)
        self.B = minIndividuel + np.random.rand(rank, num_items) * (maxIndividuel - minIndividuel)
...
class OracleNMF_small(VirtualOracle):
    def __init__(self, num_users, num_items, rank=10, min_rating = 1.0, max_rating=5.0):
        ...
        A = minIndividuel + np.random.rand(num_users, rank)*(maxIndividuel - minIndividuel)
        B = minIndividuel + np.random.rand(rank, num_items) * (maxIndividuel - minIndividuel)
